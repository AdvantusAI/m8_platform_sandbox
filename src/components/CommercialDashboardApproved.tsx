  ...
  export function CommercialDashboardApproved() {
    ...
    const handleProductSelect = async (productId: string) => {
      ...
      try {
        ...
        setSelectedProductName(product?.product_name || `Producto ${productId}`);
      } catch (error) {
        console.error('Error fetching product name:', error);
        setSelectedProductName(`Producto ${productId}`);
      }
    };
    ...
    const handleSearchByIds = async () => {
      const searchQuery = typeof idSearchQuery === 'string' ? idSearchQuery : String(idSearchQuery || '');
      const queryIds = searchQuery.split(',').map(id => id.trim()).filter(id => id.length > 0);
      
      if (queryIds.length === 0) {
        console.log('No valid product IDs to search');
        return;
      }

      try {
        console.log('Searching for product IDs:', queryIds);
        
        if (queryIds.length === 1) {
          // Single product search
          const response = await fetch(`http://localhost:3001/api/forecast-data/product/${queryIds[0]}`);
          ...
          const response = await fetch('http://localhost:3001/api/forecast-data/by-products', {
            ...
            body: JSON.stringify({ productIds: queryIds.map(id => parseInt(id)).filter(id => !isNaN(id)) }),
          });
    ...
    return <div className="space-y-6">
        ...
        {/* Selection Modals */}
        <ProductSelectionModal isOpen={isProductModalOpen} onClose={() => setIsProductModalOpen(false)} onSelect={selection => {
        if (selection.type === 'product' && selection.ids.length > 0) {
          handleProductSelect(selection.ids[0]); // Take first product ID for single selection
        }
